...
class AddIpFrame(tk.Frame):
    def __init__(self, master=None, listbox_frame=None, delete_frame=None, **kwargs) -> None:
        ...
        self.entry = tk.Entry(self, validate="key", validatecommand=(self.register(validate_ip), "%P"))
    ...
    def add_ip(self):
        def add():
            item = self.entry.get()
            if item:
                self.listbox_frame.listbox.insert(tk.END, item)
...
class DeleteIpsFrame(tk.Frame):
    ...
    def delete_all_ips(self):
        def delete():
            self.listbox_frame.listbox.delete(0, tk.END)
            # После удаления элементов обновляем состояние кнопок
            self.update_buttons_state()
        return delete
...
class StartAnalyseFrame(tk.Frame):
    def __init__(self, master=None, listbox_frame=None, time_frame=None, file_path_frame=None, **kwargs) -> None:
        ...
        self.listbox_frame.listbox.bind('<<ListboxSelect>>', self.validate_start)
        self.time_frame.set_time_var.trace_add("write", self.validate_start)
        self.file_path_frame.pcapng_entry.bind("<KeyRelease>", self.validate_start)
        self.file_path_frame.csv_entry.bind("<KeyRelease>", self.validate_start)

        # Вызываем функцию для установки начального состояния кнопок
        self.validate_start()

    def validate_start(self, *_):
        # Проверяем условия для активации кнопок
        if (self.listbox_frame.listbox.size() != 0 and 
            validate_number(self.time_frame.set_time_var.get()) and 
